...
## Montana Standards
- CS.AP.4.1 compare and refine multiple algorithms for the same task and determine which is the most appropriate.
- CS.AP.4.2 break down problems into smaller, manageable subproblems to facilitate the program development process.
- CS.AP.5.1 compare and refine multiple algorithms for the same task and determine which is the most appropriate.
...
- CS.NI.4 each student will identify cybersecurity problems.
- CS.NI.5.1 explain cybersecurity problems.
...
## Vocabulary
Encryption
: a method of securing information using one or more mathematical techniques.

Decryption
: a method of converting ciphertext back to plaintext; using a “key” to uncover the original message in an encrypted message.

Plaintext
: information that can be directly read by a human or machine. The unencrypted text.

Ciphertext
: information that has been encrypted.

Cracking
: a method of attempting to convert ciphertext to plaintext without the use of a key.
...
```
![alice bob and eve encryption triangle](../resources/images/4-2_alice-bob-eve.png)
...
![m on the cpx](../resources/4-2_m-on-cpx.png)
