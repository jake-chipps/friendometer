...
<span class="task">Program the circuit so that the par LEDs and ind LED blink each second alternatingly. In other words, when the par LEDs are on, the ind LED is off. And when the ind LED
is on, the par LEDs are off.</span>

<span class="task">Solution</span> (Hover right to view) <span class="solution">Assume parallel LEDs are connected to pin 10 (let's name the variable for pin 10 "par") and the independent LED is connected to pin 12 (let's call that variable "ind")--> digitalWrite(par, HIGH); digitalWrite(ind, LOW); delay(1000); digitalWrite(par, LOW); digitalWrite(ind, HIGH); delay(1000);</span>
...
<span class="task">Create a function below the *loop* called *alternateBlink* that programs
the parallel LEDs and independent LED to blink each second alternatingly. Call
*alternateBlink* in the *loop*. Test your code.</span>
