...
This seems like a lot of redundant. Really, all we are doing here is changing the position (index) of the NeoPixel. If only there were a way that we can write some code to loop through the numbers 0 to 9 and set the color at that index. Oh wait, there is.

### The For Loop
Here we will talk about a very particular loop that will allow us to run through the indices of the NeoPixels easily.
Indices
For loop
